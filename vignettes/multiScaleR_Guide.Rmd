...
We are now ready to use the `multiScale_optim` function. Note: for purposes of this vignette, the function is not being run in parallel. Optimization will be much quicker when parallelized by specifying `opt_parallel = TRUE` and number of cores to use with `n_cores`. The optimization below takes \~50 seconds to complete.
